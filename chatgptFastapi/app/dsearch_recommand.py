...
def search_recommand(search):
  ...
  # title 칼럼이 입력된 title_name 값인 DataFrame추출
    ...
    similar_course_list = []
    similar_course_list = {"Title" : similar_course['Title'][:10].values.tolist(), "Content" : similar_course[["Content"]][:10].values.tolist(), "Tag" : similar_course[['Tag']][:10].values.tolist(), "Author" : similar_course[['Author']][:10].values.tolist(), "index" : similar_course[['Index']][:10].values.tolist()}
