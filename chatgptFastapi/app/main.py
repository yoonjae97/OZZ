...
# def generate_description(input: DescriptionInput):
...
#     #return await run_async_function_in_thread(process)
    
class landingInput(BaseModel):
    concern: list

@app.post("/dlanding_recommand")
def dlanding_recommand(input: landingInput):
    #def process():
...
def dmission_recommand(input: missionInput):
    #def process():
...
def dsearch_recommand(input: SearchInput):
    #def process():
